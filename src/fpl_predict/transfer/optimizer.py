...
def _adjust_xmins_with_depth(
    pool: List[Player], nonstarter_xmins: float, gk_backup_xmins: float
) -> None:
...
def _cheapest_by_pos(
    pool: List[Player], pos: str, exclude_ids: Set[int], used_clubs: Dict[int, int]
) -> Optional[Player]:
...
def _choose_cheap_bench_template(pool: List[Player], bench_budget: int) -> List[Player]:
    ...
    used.append(gk)
    exclude.add(gk.id)
    clubs[gk.team] = clubs.get(gk.team, 0) + 1
    ...
    if cost > bench_budget:
        ...
        while cost > bench_budget and tries < 50:
            ...
            cands = [
                p for p in outfield if p.pos == worst.pos and p.id not in {pp.id for pp in used}
            ]
            cands.sort(key=lambda z: (z.cost, -z.ep1))
            swapped = False
            for q in cands:
...
def _repair_bench_budget(
    xi: List[Player], picked: List[Player], bench_budget: int
...
def evaluate_transfer_with_optimal_xi(
    ...
    prob += pulp.lpSum(
        [
            xi_vars[p.id] * p.eph
            + (1 - xi_vars[p.id]) * (p.eph * bench_weight + bench_xmins_weight * (p.xmins / 90.0))
...
def optimize_transfers_two_stage(
    ...
    for p in current_players:
        if p.ep_base == 0 or p.xmins == 0:
            ...
            log.info(
                f"Dead player identified: {p.name} (ID={p.id}, EP={p.ep_base:.2f}, xMins={p.xmins:.0f})"
            )
    ...
    for team_id, count in team_player_counts.items():
        if count > MAX_PER_CLUB:
            ...
            team_name = (
                team_players[team_id][0].name.split()[0]
                if team_players[team_id]
                else f"Team {team_id}"
            )
            log.warning(
                f"🚨 CLUB LIMIT VIOLATION: {count} players from team {team_id} (max {MAX_PER_CLUB})"
            )
    ...
    if club_violation_player_ids:
        log.warning(
            f"Found {len(club_violation_player_ids)} player(s) from clubs over limit - MUST transfer one out"
        )
    ...
    if os.path.exists(myteam_file):
        ...
        for pick in myteam_data.get("picks", []):
            selling_prices[pick["element"]] = pick.get(
                "selling_price", pick.get("purchase_price", 0)
            )
    ...
    if max_transfers == 1:
            ...
            for p_in in candidates:
                ...
                if fixes_club_violation:
                    ...
                    if p_in.team != p_out.team:
                        ...
                        log.info(
                            f"🚨 Club violation fix: {p_out.name} -> {p_in.name}, base gain={ep_gain - club_violation_bonus:.2f}, with bonus={ep_gain:.2f}"
                        )
                ...
                if is_removing_dead and not fixes_club_violation:
                    ...
                    log.info(
                        f"Dead player transfer: {p_out.name} -> {p_in.name}, base gain={ep_gain - dead_player_bonus:.2f}, with bonus={ep_gain:.2f}"
                    )
        ...
        for p_out in current_players:
            ...
            premium_downgrades = [
                ...
                if selling_price > 100  # If selling premium (>£10m)
                and p.cost < selling_price  # Cheaper option
                and p.cost > selling_price - 50
            ]  # But not too cheap (within £5m)
            ...
            for p_in in candidates:
                ...
                if (
                    p_in.cost > available_budget + 50
                ):  # Allow up to £5m more expensive with savings elsewhere
                    continue
        ...
        if single_transfers:
            log.info(
                f"Top single transfer: {single_transfers[0]['out'].name} -> {single_transfers[0]['in'].name}, EP gain: {single_transfers[0]['ep_gain']:.2f}"
            )
        ...
        log.info(
            f"Found {len(downgrades)} downgrades, {len(upgrades)} upgrades, {len(neutral)} neutral"
        )
        ...
        for t1 in downgrades[:20]:  # Top 20 downgrades
            for t2 in upgrades[:20]:  # Top 20 upgrades
                ...
                if combinations_tried <= 5:
                    log.info(
                        f"Combo {combinations_tried}: {t1['out'].name}->{t1['in'].name} + {t2['out'].name}->{t2['in'].name}"
                    )
                    log.info(
                        f"  Budget: Out £{total_out / 10:.1f}m, In £{total_in / 10:.1f}m, Valid: {total_in <= total_out}"
                    )
        ...
        log.info(
            f"Tried {combinations_tried} combinations, found {combinations_valid} valid 2-transfer options"
        )
        log.info(
            f"Added {min(20, len([t for t in single_transfers[:20] if t['budget_diff'] >= 0]))} single transfer options"
        )
        ...
        log.info(
            f"Two-stage optimization not implemented for {max_transfers} transfers, using regular LP"
        )
    ...
    for i, option in enumerate(transfer_options):
        ...
        if i < 10:  # Log top 10 for debugging
            if len(option["transfers"]) == 1:
                ...
                log.info(
                    f"Evaluating: {p_out.name} -> {p_in.name} (EP gain: {option['ep_gain']:.2f})"
                )
            else:
                transfers_str = ", ".join(
                    [f"{out.name}->{inp.name}" for out, inp in option["transfers"]]
                )
        ...
        prob += pulp.lpSum(
            [
                xi_vars[p.id] * p.eph
                + (1 - xi_vars[p.id])
                * (p.eph * bench_weight + bench_xmins_weight * (p.xmins / 90.0))
                for p in squad_players
            ]
        )
        ...
        if prob.status == pulp.LpStatusOptimal:
            ...
            if i < 10:
                log.info(
                    f"  Objective after transfer(s): {objective:.2f} (gain: {objective - baseline_objective:.2f})"
                )
    ...
    if best_option and best_objective > baseline_objective:
        if len(best_option["transfers"]) == 1:
            ...
            transfers_str = ", ".join(
                [f"{out.name}->{inp.name}" for out, inp in best_option["transfers"]]
            )
...
def optimize_with_lp(
    ...
    **kwargs,
    ...
    try:
        ...
        # Load data-driven team quality scores once. They are keyed on canonical slugs, so
        # the FPL team id has to be translated before the lookup: matching on the display
        # name silently missed every time and left every player at the 1.0 default.
        ...
        team_slug_map = bootstrap_team_slugs(js)
        n_matched = sum(1 for s in team_slug_map.values() if s in team_quality_scores)
        log.info("Team quality scores resolved for %d/%d clubs", n_matched, len(team_slug_map))
        ...
        if current_squad and max_transfers > 0 and not wildcard:
            ...
            if os.path.exists(myteam_file):
                ...
                log.info(
                    f"Budget: Selling value={selling_value / 10:.1f}m + Bank={bank / 10:.1f}m = {budget_limit / 10:.1f}m"
                )
            ...
            for e in js["elements"]:
                ...
                # Get team quality, via the canonical slug the scores are keyed on
                team_strength = team_quality_scores.get(team_slug_map.get(team_id, ""), 1.0)
                ...
                players.append(
                    Player(
                        ...
                        is_differential=float(e.get("selected_by_percent", 0)) < 10.0
                        and ep_base > 3.0,
            ...
            if result is None:
                log.info(
                    f"Two-stage optimization not available for {max_transfers} transfers, falling back to regular LP"
                )
                ...
                if "transfer_out" in result and result["transfer_out"]:
                    ...
                    output.append(
                        f"OUT: {p_out.name} ({p_out.pos}) - £{p_out.cost / 10:.1f}m, EP: {p_out.eph:.1f} over {horizon} GWs ({p_out.ep1:.2f}/wk)"
                    )
                    output.append(
                        f"IN:  {p_in.name} ({p_in.pos}) - £{p_in.cost / 10:.1f}m, EP: {p_in.eph:.1f} over {horizon} GWs ({p_in.ep1:.2f}/wk)"
                    )
                else:
                    # Multiple transfers
                    for i, (p_out, p_in) in enumerate(
                ...
                if result["captain"]:
                    output.append(
                        f"Captain: {result['captain'].name} - {result['captain'].ep1:.2f} pts (doubled = {result['captain'].ep1 * 2:.2f})"
                    )
                if vice_captain:
                ...
                return {
                    ...
                    "transfers_out": [
                        p.id for p in result.get("transfers_out", [result.get("transfer_out")])
                    ]
                    if result.get("transfers_out") or result.get("transfer_out")
                    else [],
                    "transfers_in": [
                        p.id for p in result.get("transfers_in", [result.get("transfer_in")])
                    ]
                    if result.get("transfers_in") or result.get("transfer_in")
                    else [],
        ...
        for e in js["elements"]:
            ...
            # Data-driven team quality score (0.5-1.5, 1.0 average), keyed on canonical slug
            team_strength = team_quality_scores.get(team_slug_map.get(team_id, ""), 1.0)
        ...
        try:
            ...
            for p in players:
                player_data.append(
                    {"id": p.id, "player_id": p.id, "EPH": p.eph, "position": p.pos, "cost": p.cost}
                )
        ...
        if current_squad and max_transfers > 0:
            ...
            if virgil:
                log.info(
                    f"  Virgil (373): EPH={virgil.eph:.2f}, Cost=£{virgil.cost / 10:.1f}m - IN POOL"
                )
            else:
                log.info(f"  Virgil (373): NOT IN POOL")
            if xhaka:
                log.info(
                    f"  Xhaka (668): EPH={xhaka.eph:.2f}, Cost=£{xhaka.cost / 10:.1f}m - IN POOL"
                )
            else:
                log.info(f"  Xhaka (668): NOT IN POOL")
            if frimpong:
                log.info(
                    f"  Frimpong (370): EPH={frimpong.eph:.2f}, Cost=£{frimpong.cost / 10:.1f}m - IN POOL"
                )
            else:
                log.info(f"  Frimpong (370): NOT IN POOL")
            if reijnders:
                log.info(
                    f"  Reijnders (427): EPH={reijnders.eph:.2f}, Cost=£{reijnders.cost / 10:.1f}m - IN POOL"
                )
            else:
        ...
        for p in players:
            ...
            if p in debug_players:
                log.info(
                    f"Objective for {p.name} ({p.id}): EPH={p.eph:.2f}, "
                    + f"EP_contrib={ep_contrib:.2f}, XI_contrib={xi_contrib:.2f}"
                )

            objective_terms.append(
                xi_vars[p.id] * xi_contrib + captain_var[p.id] * cap_bonus + bench_contrib
            )
        ...
        if not current_squad:
            ...
            for formation, reqs in FORMATIONS.items():
                for pos, required in reqs.items():
                    prob += pulp.lpSum(
                        [xi_vars[p.id] for p in players if p.pos == pos]
                    ) >= required - 100 * (1 - formation_vars[formation])
                    prob += pulp.lpSum(
                        [xi_vars[p.id] for p in players if p.pos == pos]
                    ) <= required + 100 * (1 - formation_vars[formation])
        ...
        if budget_limit:
            ...
            if os.path.exists(myteam_file):
                ...
                log.info(
                    f"Budget calculation: Selling value={selling_value / 10:.1f}m + Bank={bank / 10:.1f}m = Total={budget_limit / 10:.1f}m"
                )
        ...
        if not (current_squad and max_transfers == 0):
            bench_cost = pulp.lpSum([squad_vars[p.id] * p.cost for p in players]) - pulp.lpSum(
                [xi_vars[p.id] * p.cost for p in players]
            )
        ...
        for team_id in set(p.team for p in players):
            prob += (
                pulp.lpSum([squad_vars[p.id] for p in players if p.team == team_id]) <= MAX_PER_CLUB
            )
            ...
            if max_transfers == 0:
                ...
                for p in players:
                    ...
                    if p.id in current_squad:
                        ...
                        prob += (
                            transfer_in_vars[p.id] == 0
                        )  # Can't transfer in a player we already have
                        ...
                        prob += (
                            transfer_out_vars[p.id] == 0
                        )  # Can't transfer out a player we don't have
                ...
                for pos in ["GKP", "DEF", "MID", "FWD"]:
                    transfers_out_pos = pulp.lpSum(
                        [transfer_out_vars[p.id] for p in players if p.pos == pos]
                    )
                    transfers_in_pos = pulp.lpSum(
                        [transfer_in_vars[p.id] for p in players if p.pos == pos]
                    )
                ...
                if max_transfers > 0:
                    ...
                    for p in players:
                        if p.id not in current_squad:  # Potential transfer target
                            ...
                            pos_players_current = [
                                pl for pl in players if pl.pos == p.pos and pl.id in current_squad
                            ]
                            if pos_players_current:
                                avg_ep_current = sum(pl.eph for pl in pos_players_current) / len(
                                    pos_players_current
                                )
        ...
        solver = pulp.PULP_CBC_CMD(
            msg=1
            if current_squad and max_transfers > 0
            else 0,  # Show solver output for debugging transfers
        ...
        if current_squad and max_transfers > 0:
            if prob.status == pulp.LpStatusOptimal:
                log.info(
                    f"Transfer optimization completed successfully. Objective value: {pulp.value(prob.objective):.2f}"
                )
            ...
            for p in current_defs:
                ...
                log.info(
                    f"  {p.name} ({p.team}) ID={p.id} - EP={p.ep_base:.2f}, EPH={p.eph:.2f}, Cost=£{p.cost / 10}m -> {selected}"
                )

            # Log key Liverpool defenders for debugging Frimpong/Virgil issue
            liverpool_defs = [
                p for p in players if p.pos == "DEF" and p.team == 12
            ]  # Liverpool team ID
            ...
            for p in sorted(liverpool_defs, key=lambda x: x.eph, reverse=True)[:5]:  # Top 5 by EPH
                ...
                log.info(
                    f"  {p.name} (ID:{p.id}) - EP={p.ep_base:.2f}, EPH={p.eph:.2f}, Team strength={p.team_strength:.2f}, Cost=£{p.cost / 10}m - {in_squad} -> {selected}"
                )

            # Check Liverpool player count
            liverpool_count = sum(
                1 for p in players if p.team == 12 and squad_vars[p.id].varValue > 0.5
            )
            log.info(f"Liverpool players in final squad: {liverpool_count}/3 max")
            liverpool_in_squad = [
                p.name for p in players if p.team == 12 and squad_vars[p.id].varValue > 0.5
            ]
            ...
            for p_out in [p for p in players if p.id in current_squad]:
                for p_in in [
                    p for p in players if p.id not in current_squad and p.pos == p_out.pos
                ]:
            ...
            for ep_gain, p_out, p_in in sorted(
                transfer_opportunities, key=lambda x: x[0], reverse=True
            )[:10]:
                log.info(
                    f"  {p_out.name} -> {p_in.name}: +{ep_gain:.2f} EP (£{p_out.cost / 10:.1f}m -> £{p_in.cost / 10:.1f}m)"
                )
            ...
            for p in players:
                if (
                    p.id in current_squad
                    and xi_vars[p.id].varValue
                    and xi_vars[p.id].varValue > 0.5
                ):
            ...
            for p in players:
                if p.id in current_squad and squad_vars[p.id].varValue < 0.5:
                    ...
                    log.info(
                        f"OUT: {p.name} ({p.pos}) ID={p.id} - EP over {horizon} GWs: {p.eph:.2f}, Cost: £{p.cost / 10:.1f}m"
                    )
                elif p.id not in current_squad and squad_vars[p.id].varValue > 0.5:
                    transfers_made.append(("IN", p))
                    log.info(
                        f"IN: {p.name} ({p.pos}) ID={p.id} - EP over {horizon} GWs: {p.eph:.2f}, Cost: £{p.cost / 10:.1f}m"
                    )
        ...
        if not current_squad:
            ...
            for f, var in formation_vars.items():
                if var.varValue and var.varValue > 0.5:
                    ...
                    xi_players = [
                        p
                        for p in players
                        if xi_vars[p.id].varValue and xi_vars[p.id].varValue > 0.5
                    ]
            ...
            xi_players = [
                p for p in players if xi_vars[p.id].varValue and xi_vars[p.id].varValue > 0.5
            ]
            if xi_players:
                total_xi_ep = sum(p.ep1 for p in xi_players)
                pos_breakdown = {}
                for p in xi_players:
                    pos_breakdown[p.pos] = pos_breakdown.get(p.pos, 0) + 1
                log.info(
                    f"XI composition after transfers: {pos_breakdown}, Total EP: {total_xi_ep:.2f}"
                )
            ...
            if prob.status == pulp.LpStatusInfeasible:
                ...
                if current_squad:
                    ...
                    if current_positions != POSITIONS:
                        log.warning(
                            f"Position mismatch! Required: {POSITIONS}, Current: {current_positions}"
                        )
        ...
        expected_points = sum(p.ep1 for p in xi) + (
            next(p.ep1 for p in xi if p.id == captain_id) if captain_id else 0
        )
    ...
    **kwargs,
    ...
    if use_myteam and not current_squad:
        ...
        if os.path.exists(myteam_file):
            ...
            if wildcard:
                ...
                selling_value = sum(
                    pick.get("selling_price", pick.get("purchase_price", 0))
                    for pick in myteam_data.get("picks", [])
                )
                bank = myteam_data.get("transfers", {}).get("bank", 0)
                ...
                log.info(
                    f"Wildcard mode: Budget = £{wildcard_budget / 10:.1f}m (selling value £{selling_value / 10:.1f}m + bank £{bank / 10:.1f}m)"
                )
    ...
    if current_squad and max_transfers == 0:
        ...
        for element in js["elements"]:
            if element["id"] in current_squad:
                pid = element["id"]
                xmins = xmins_map.get(
                    pid, element.get("minutes", 0) / 38 * 90 if element.get("minutes") else 45.0
                )
        ...
        for form in formations_to_try:
            ...
            if (
                len(by_position["GKP"]) >= 1
                and len(by_position["DEF"]) >= n_def
                and len(by_position["MID"]) >= n_mid
                and len(by_position["FWD"]) >= n_fwd
            ):
        ...
        if captain:
            output.append(
                f"Captain: {captain['name']} - {captain['ep']:.2f} pts (doubled = {captain['ep'] * 2:.2f})"
            )
        if vice_captain:
    ...
    if use_advanced and HAS_PULP:
        ...
        if result and "error" not in result:
            ...
            if result:
                log.warning(
                    f"LP optimization failed: {result.get('error', 'Unknown error')}. Falling back to greedy."
                )
            else:
                log.warning("LP optimization returned None. Falling back to greedy.")
    elif use_advanced and not HAS_PULP:
        log.warning(
            "PuLP not installed. Install with 'pip install pulp' for LP optimization. Using greedy approach."
        )
    ...
    _adjust_xmins_with_depth(
        pool, nonstarter_xmins=float(nonstarter_xmins), gk_backup_xmins=float(gk_backup_xmins)
    )
    ...
    allowed_c_pos = {p.strip().upper() for p in captain_positions.split(",") if p.strip()} or {
        "MID",
        "FWD",
    }
    allowed_v_pos = {p.strip().upper() for p in vice_positions.split(",") if p.strip()} or {
        "MID",
        "FWD",
        "DEF",
    }
